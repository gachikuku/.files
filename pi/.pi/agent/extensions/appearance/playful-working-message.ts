...
function useNormalThinkingText(component: AssistantMessageComponent): void {
  const { contentContainer } =
    component as unknown as AssistantMessageInternals;
...
function makeClaudeStyleFrame(
  ...
  const animatedMessage = characters
    ...
    .join("");
  const spinner =
    CLAUDE_SPINNER[Math.floor(frameIndex / 2) % CLAUDE_SPINNER.length]!;
...
export default function (pi: ExtensionAPI) {
  const assistantPrototype =
    AssistantMessageComponent.prototype as unknown as AssistantMessagePrototype;
  assistantPrototype[ORIGINAL_ASSISTANT_UPDATE_MARK] ??=
  ...
  pi.on("before_agent_start", (_event, ctx) => {
    ...
    const updateAnimation = () => {
      ...
      ctx.ui.setWorkingIndicator({
        frames: [
          makeClaudeStyleFrame(workingMessage, frameIndex, elapsedSeconds),
        ],
      });
      frameIndex++;
    };
