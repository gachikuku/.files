...
export class Union extends Base {
  ...
  getParser(recur) {
    ...
    return (oinput, options) => {
      ...
      if (!eff) {
        return state.out ? Effect.succeed(state.out) : Effect.fail(new SchemaIssue.AnyOf(ast, input, state.issues ?? []));
      }
...
export function makeFilter(filter, annotations, aborted = false) {
  return new Filter((input, ast, options) => SchemaIssue.make(input, ast, filter(input, ast, options)), annotations, aborted);
}
/** @internal */
export function makeFilterByGuard(is, annotations) {
  return new Filter(input => is(input) ? undefined : new SchemaIssue.InvalidValue(Option.some(input)), annotations, true // after a guard, we always want to abort
  );
}
/**
 ...
 */
const symbolToString = /*#__PURE__*/new Link(symbolString, /*#__PURE__*/new SchemaTransformation.Transformation(/*#__PURE__*/SchemaGetter.transform(description => globalThis.Symbol.for(isStringSymbolRegExp.exec(description)[1])), /*#__PURE__*/SchemaGetter.transformOrFail(sym => {
  const key = globalThis.Symbol.keyFor(sym);
  if (key !== undefined) {
    return Effect.succeed(globalThis.String(sym));
  }
  return Effect.fail(new SchemaIssue.Forbidden(Option.some(sym), {
...
/** @internal */
export const Json = /*#__PURE__*/new Declaration([], () => (input, ast) => isJson(input) ? Effect.succeed(input) : Effect.fail(new SchemaIssue.InvalidType(ast, Option.some(input))), {
  typeConstructor: {
    _tag: "effect/Json"
  },
  generation: {
