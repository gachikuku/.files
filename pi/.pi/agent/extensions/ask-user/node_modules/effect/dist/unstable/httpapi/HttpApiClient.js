...
export const makeWith = (api, options) => {
  ...
  return makeClient(api, {
    ...
    onEndpoint({
      ...
      InternalRecord.set(group.topLevel ? client : client[group.identifier], endpoint.identifier, endpointFn);
    }
...
export const urlBuilder = (api, options) => {
  ...
  HttpApi.reflect(api, {
    ...
    onEndpoint({
      ...
      InternalRecord.set(group.topLevel ? builder : builder[group.identifier], endpoint.identifier, endpointBuilder);
    }
  });
  return builder;
};
...
function groupSchemasByContentType(schemas) {
  ...
  for (const schema of schemas) {
    const contentType = HttpApiSchema.isNoContent(schema.ast) ? "" : MediaType.normalize(HttpApiSchema.getResponseEncoding(schema.ast).contentType);
...
function streamToResponse(streamSchema) {
  ...
  return response => Effect.map(Effect.context(), context => Stream.provideContext(sse === undefined ? response.stream : decodeSseStream(response.stream, sse.declaration, sse.decoder), context));
}
      ...
      case "Json":
        {
          ...
          return UnknownFromArrayBuffer.pipe(Schema.decodeTo(schema, encodedIsNull ? SchemaTransformation.transform({
...
function getEncodePayloadSchemaFromBody(schema, method) {
  ...
  const out = $HttpBody.pipe(Schema.decodeTo(schema, SchemaTransformation.transformOrFail({
    ...
    encode(t) {
      switch (encoding._tag) {
        case "Multipart":
          return Effect.fail(new SchemaIssue.Forbidden(Option.some(t), {
