...
const $IsGUID = /*#__PURE__*/Schema.Struct({
 ...
 * @since 4.0.0
 */
export const $StringMeta = /*#__PURE__*/Schema.Union([$IsStringFinite, $IsStringBigInt, $IsStringSymbol, $IsTrimmed, $IsUUID, $IsGUID, $IsULID, $IsBase64, $IsBase64Url, $IsStartsWith, $IsEndsWith, $IsIncludes, $IsUppercased, $IsLowercased, $IsCapitalized, $IsUncapitalized, $IsMinLength, $IsMaxLength, $IsPattern, $IsLengthBetween]).annotate({
  ...
  if (Predicate.isObject(typeConstructor) && typeof typeConstructor._tag === "string") {
    ...
    switch (typeConstructor._tag) {
      ...
      case "Date":
        return Schema.Date;
      case "Error":
        return Schema.Error(typeConstructor.options);
      case "File":
        return Schema.File;
      case "FormData":
      ...
      case "effect/Redacted":
        return Schema.Redacted(typeParameters[0], typeConstructor.options);
...
export function fromJsonSchemaMultiDocument(document, options) {
  ...
  function unknownToJson(representation) {
    switch (representation._tag) {
      case "Unknown":
        return representation.annotations === undefined ? json : {
          ...json,
          annotations: {
            ...json.annotations,
      ...
      case "Suspend":
        {
          ...
          return thunk === representation.thunk ? representation : {
            ...representation,
            thunk
          };
        }
      case "String":
        {
          ...
          return contentSchema === representation.contentSchema ? representation : {
            ...representation,
            contentSchema
          };
        }
      case "Arrays":
        {
          ...
          return elements === representation.elements && rest === representation.rest ? representation : {
            ...representation,
            elements,
            rest
          };
      ...
      case "Objects":
        {
          const propertySignatures = SchemaAST.mapOrSame(representation.propertySignatures, propertySignature => {
            ...
            return type === propertySignature.type ? propertySignature : {
              ...propertySignature,
              type
            };
          });
          const indexSignatures = SchemaAST.mapOrSame(representation.indexSignatures, indexSignature => {
            ...
            return type === indexSignature.type ? indexSignature : {
              ...indexSignature,
              type
            };
          });
          return propertySignatures === representation.propertySignatures && indexSignatures === representation.indexSignatures ? representation : {
            ...representation,
            propertySignatures,
            indexSignatures
          };
      ...
      case "Union":
        {
          ...
          return types === representation.types ? representation : {
            ...representation,
            types
          };
        }
