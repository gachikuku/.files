...
export function forbidden(message) {
  return fail(oe => new SchemaIssue.Forbidden(oe, {
    message: message(oe)
  }));
}
...
export function parseJson(options) {
  return onSome(input => Effect.try({
    ...
    catch: e => new SchemaIssue.InvalidValue(Option.some(input), {
...
export function decodeBase64() {
  return transformOrFail(input => Effect.mapErrorEager(Effect.fromResult(Encoding.decodeBase64(input)), e => new SchemaIssue.InvalidValue(Option.some(input), {
    message: e.message
  })));
}
/**
...
export function decodeBase64String() {
  return transformOrFail(input => Result.match(Encoding.decodeBase64String(input), {
    onFailure: e => Effect.fail(new SchemaIssue.InvalidValue(Option.some(input), {
      message: e.message
    })),
...
export function dateTimeUtcFromInput() {
  return transformOrFail(input => {
    return Option.match(DateTime.make(input), {
      onNone: () => Effect.fail(new SchemaIssue.InvalidValue(Option.some(input), {
